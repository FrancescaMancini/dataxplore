...
mod_info_tab_server <- function(id){
  moduleServer( id, function(input, output, session){
    ...
    ex_tab <- data.frame(species = c("a", "b", "c", "d"),
                         x = c(-4.629295227, -2.541747496, -0.080339407, -2.599618498),
                         y = c(52.87884242, 51.74732335, 52.98631657, 52.30386493),
                         date = c("27/10/2017", "30/01/2018", "01/07/2016", "03/06/2015"),
                         year = c("2017", "2018", "2016", "2015"),
                         group = c("A", "A", "C", "C"),
                         sp_uncertainty = c("100", "10000", "1000", "100"))

    output$table <- renderTable({ex_tab})
